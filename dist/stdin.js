...
export async function readStdin() {
    ...
    return new Promise((resolve) => {
        ...
        const onData = (chunk) => { raw += String(chunk); };
...
/** 格式化 Bedrock 模型名称 */
function normalizeBedrockModel(modelId) {
    const lower = modelId.toLowerCase();
    ...
    const familyIdx = tokens.findIndex((t) => t === 'haiku' || t === 'sonnet' || t === 'opus');
    if (familyIdx === -1)
        return modelId;
...
export function getSessionTokens(stdin) {
    const usage = stdin.context_window?.current_usage;
    if (!usage)
        return null;
