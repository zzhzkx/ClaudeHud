...
- **📁 项目路径** — 当前工作目录
- **🌿 Git 状态** — 当前分支和脏状态指示
- **⏱ 会话时长** — 本次会话已用时间（精确到秒，如 `2h 15m 47s`）
...
**窗口内没有生成活动时（你在打字、工具在跑、思考中）显示 `🚀 — t/s`**，
而不是整行消失 —— 一个会闪没的读数会让人以为功能坏了。
...
所以**别把 `refreshInterval` 设成 1 秒**：实测单次渲染 206ms（几乎全是进程启动和
I/O，真正算时长的部分不到 1ms），1 秒一次等于长期占用约 21% 的单个核。默认
`10` 秒约 2%。
...
会话时长精确到秒，但默认只在事件驱动时重绘 —— 秒数会跳着变。要让秒数稳定走动，
在 `~/.claude/settings.json` 的 `statusLine` 里加 `refreshInterval`（单位秒，≥1）：
...
这个值参考 [ccstatusline](https://github.com/sirmalloc/ccstatusline) 的默认（10 秒，
可配 1–60 秒）。**不建议设成 1**，原因见上一节 —— 本项目是每次重绘新起进程，
不是常驻进程，1 秒刷新会长期占用约 21% 的单个核。
...
```
🤖 [LongCat-2.0-Preview] │ 📁 ClaudeHud │ 🌿 main ● │ ⏱ 2h 15m 47s
